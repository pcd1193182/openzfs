...
typeset recv_vol_file="/dev/zvol/rdsk/$recvvol"
...
if [[ ! -c $recv_vol_file ]]; then
	for t in 2 2 1 1 1; do
		log_note "Polling $t seconds for device file."
		sleep $t
		[[ -c $recv_vol_file ]] && break
	done
fi
[[ -c $recv_vol_file ]] || log_fail "Volume device file should exist."
